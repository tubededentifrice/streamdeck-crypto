...
(() => {
  ...
  var require_formatters = __commonJS({
    "com.courcelle.cryptoticker-dev.sdPlugin/js/formatters.ts"(exports, module) {
      ...
      (function(root, factory) {
        ...
        const COMPACT_UNITS = [
          { value: 1e3 * 1e3 * 1e3 * 1e3, suffix: "T" },
          { value: 1e3 * 1e3 * 1e3, suffix: "B" },
          { value: 1e3 * 1e3, suffix: "M" },
          { value: 1e3, suffix: "K" }
        ];
        const DEFAULT_THOUSANDS_SEPARATOR = ",";
        const DEFAULT_DECIMAL_SEPARATOR = ".";
        function resolveSeparators(options) {
          const thousandsCandidate = options && typeof options.thousandsSeparator === "string" ? options.thousandsSeparator : null;
          const decimalCandidate = options && typeof options.decimalSeparator === "string" ? options.decimalSeparator : null;
          const thousandsSeparator = thousandsCandidate && thousandsCandidate.length > 0 ? thousandsCandidate : DEFAULT_THOUSANDS_SEPARATOR;
          const decimalSeparator = decimalCandidate && decimalCandidate.length > 0 ? decimalCandidate : DEFAULT_DECIMAL_SEPARATOR;
          return {
            thousandsSeparator,
            decimalSeparator
          };
      ...
      (function(root, factory) {
        ...
        function renderTickerCanvas(params) {
          ...
          if (!shouldDisplayDetails) {
            ...
            const priceText = priceValue !== null ? formatters.getRoundedValue(priceValue, digits, multiplier, priceFormat, numericSeparators) : "--";
            ...
            if (settings["displayHighLow"] !== "off") {
              ...
              canvasContext.fillStyle = textColor;
              canvasContext.textAlign = "left";
              const lowText = lowValue !== null ? formatters.getRoundedValue(lowValue, highLowDigits, multiplier, priceFormat, numericSeparators) : "--";
              canvasContext.fillText(lowText, textPadding, 90 * sizeMultiplier);
              canvasContext.textAlign = "right";
              const highText = highValue !== null ? formatters.getRoundedValue(highValue, highLowDigits, multiplier, priceFormat, numericSeparators) : "--";
  ...
  var require_ticker = __commonJS({
    "com.courcelle.cryptoticker-dev.sdPlugin/js/ticker.ts"(exports, module) {
      ...
      tickerAction2.defaultSettings = defaultSettings;
      tickerAction2.applyDefaultSettings = applyDefaultSettings;
      tickerAction2.getDefaultSettings = function() {
        return getDefaultSettingsSnapshot();
      };
      tickerAction2.settingsSchema = settingsSchema;
      function connectElgatoStreamDeckSocket(inPort, pluginUUID, inRegisterEvent, _inApplicationInfo, _inActionInfo) {
        ...
        function registerPlugin(inPluginUUID) {
          ...
          tickerAction2.websocketSend(JSON.stringify(json));
        }
        ;
        ...
        websocket.onmessage = async function(evt) {
          ...
          if (event == "keyDown") {
            await tickerAction2.onKeyDown(context, settings, coordinates, userDesiredState);
          } else if (event == "keyUp") {
            await tickerAction2.onKeyUp(context, settings, coordinates, userDesiredState);
          } else if (event == "willAppear") {
            await tickerAction2.onWillAppear(context, settings, coordinates);
          } else if (settings != null) {
            tickerAction2.refreshSettings(context, settings);
            tickerAction2.refreshTimer(context, settings);
          }
        };
        websocket.onclose = function() {
        };
        setInterval(async function() {
          await tickerAction2.refreshTimers();
        }, 3e5);
      }
      if (screenshotMode) {
        loggingEnabled = true;
        tickerAction2.connect();
        ...
        setTimeout(function() {
          tickerAction2.onWillAppear(context, settings, coordinates);
          setInterval(async function() {
            await tickerAction2.onKeyDown(context, settings, coordinates, userDesiredState);
            await tickerAction2.onKeyUp(context, settings, coordinates, userDesiredState);
          }, 5e3);
  ...
  // com.courcelle.cryptoticker-dev.sdPlugin/dev/preview.js
  var DEFAULT_RANGE = 1;
  var SAMPLE_CANDLE_COUNT = 20;
  var CANDLE_MARGIN_FACTOR = 0.05;
  var globalScope = typeof window !== "undefined" ? window : globalThis;
  var defaultsModule = typeof CryptoTickerDefaults !== "undefined" ? CryptoTickerDefaults : null;
  var previewTickerAction = typeof tickerAction !== "undefined" ? tickerAction : null;
  if (!previewTickerAction) {
    throw new Error("Ticker action dependency is missing for the preview page");
  }
  var previewBaseDefaults = defaultsModule && typeof defaultsModule.getDefaultSettings === "function" ? defaultsModule.getDefaultSettings() : typeof previewTickerAction.getDefaultSettings === "function" ? previewTickerAction.getDefaultSettings() : {};
  var applyPreviewDefaults = defaultsModule && typeof defaultsModule.applyDefaults === "function" ? function(partial) {
